...
function useSwapCallArguments(
    currency: Currency | undefined, // the coin
    ...
    }, [account, allowedSlippage, chainId, deadline, library, recipient, amount, currency])
}
...
export function useSwapCallback(
    ...
    const swapCalls = useSwapCallArguments(currency, amount, allowedSlippage, recipientAddressOrName)
    ...
    return useMemo(() => {
        if (!amount || !currency || !library || !account || !chainId) {
        ...
        return {
            ...
            callback: async function onSwap(): Promise<string> {
                ...
                return contract[methodName](...args, {
                    ...
                    .then((response: any) => {
                        const inputSymbol = currency.getSymbol(chainId)
                        const inputAmount = amount.toSignificant(3)

                        const base = `Move ${inputAmount} ${inputSymbol} `
                        const withRecipient =
                            recipient === account
                                ? base
                                : `${base} to ${
                    ...
                    .catch((error: any) => {
                        ...
                        if (error?.code === 4001) {
                            ...
                            console.error(`Bridge failed`, error, methodName, args, value)
    ...
    }, [currency, amount, library, account, chainId, recipient, recipientAddressOrName, swapCalls, addTransaction])
}
