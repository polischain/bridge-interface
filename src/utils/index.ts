...
const builders = {
    ...
    matic: (chainName: string, data: string, type: 'transaction' | 'token' | 'address' | 'block') => {
        const prefix = `https://${chainName}.polygonscan.com`
        switch (type) {
            case 'transaction':
                return `${prefix}/tx/${data}`
            case 'token':
    ...
    sparta: (chainName = '', data: string, type: 'transaction' | 'token' | 'address' | 'block') => {
        const prefix = 'https://sparta-explorer.polis.tech'
        switch (type) {
            case 'transaction':
                return `${prefix}/tx/${data}`
            case 'block':
...
export function getBridgeAddress(chainId?: ChainId) {
    if (!chainId) {
        throw Error(`Undefined 'chainId' parameter '${chainId}'.`)
    }
